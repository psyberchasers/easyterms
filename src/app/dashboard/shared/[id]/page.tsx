...
export default function SharedContractPage() {
  ...
  // Build signing URL from token
  const signingUrl = share?.documenso_signing_token
    ? `https://app.documenso.com/sign/${share.documenso_signing_token}`
    : null;

  // Handle signing completion
  const handleSigningComplete = async () => {
    // Refresh the share data to get updated status
    if (share) {
      ...
      if (updatedShare) {
        setShare(prev => prev ? { ...prev, ...updatedShare, status: "signed" } : null);
      }
    }
    router.refresh();
  };
    ...
    <div className="flex flex-col h-full bg-card overflow-hidden">
      ...
      <div className="shrink-0 px-4 py-2 bg-purple-500/5 border-b border-purple-500/10 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          {share.permission === "sign" && share.status !== "signed" && signingUrl && (
            <button
          ...
          {share.permission === "sign" && share.status !== "signed" && !signingUrl && (
            <span className="h-7 px-3 text-[11px] font-medium bg-muted text-muted-foreground rounded-md flex items-center gap-1.5">
              <PenTool className="w-3 h-3" />
              Awaiting signature setup...
            </span>
          )}
          {share.status === "signed" && (
