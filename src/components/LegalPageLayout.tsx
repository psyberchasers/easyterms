...
const LegalPageLayout = ({
  ...
  return (
    <div className={cn("min-h-screen p-4 lg:p-12 bg-background", className)}>
      <div className="pt-[60px] lg:pt-[20px]">
        <span className="inline-block px-3 py-1.5 text-sm font-medium text-purple-400 bg-purple-500/10 border border-purple-500/20 rounded-full">
          {title}
        </span>
      </div>
              ...
              <a href={`#${term.id}`}>
                {activeTerm === index && (
                  <motion.span
                    ...
                    className="bg-purple-500 absolute -left-[1.5px] top-1/2 inline-block h-5 w-[2px] -translate-y-1/2 rounded-2xl"
                    transition={{
                      type: "spring",
                      stiffness: 400,
                      damping: 30,
