...
export function PDFViewerWithSearch({ 
  ...
  useEffect(() => {
    if (!searchText || !pdfDocRef.current || numPages === 0) {
      setMatch(null);
      return;
    }
  ...
  return (
    <div className={cn("flex flex-col h-full bg-muted/20", className)}>
      {/* Controls */}
      <div className="flex items-center justify-between px-4 py-2 bg-muted/30 border-b border-border/50 shrink-0 gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => setCurrentPage(p => Math.min(numPages, p + 1))} disabled={currentPage >= numPages}>
        ...
        {searchText && (
          <div className={cn(
            ...
            match ? "bg-amber-500/20 text-amber-400" : "bg-red-500/20 text-red-400"
          )}>
            {searching ? (
              <>
              ...
              <>
                ...
                <span>Found on page {match.pageNum}</span>
              </>
            ) : (
              <>
      ...
      <div ref={scrollContainerRef} className="flex-1 overflow-auto flex justify-center p-4">
        ...
        <Document
          file={fileUrl}
          onLoadSuccess={(pdf) => { pdfDocRef.current = pdf; onDocumentLoadSuccess(pdf); }}
        ...
        >
          <div ref={pageContainerRef} className="relative inline-block">
            ...
            {/* Highlight overlay - pulsing animation to make it visible */}
            {searchText && match && match.pageNum === currentPage && (
              ...
              >
                {/* Main highlight bar - soft glow, no borders */}
                <div 
                  className="absolute inset-0"
                  style={{
                    background: 'linear-gradient(90deg, transparent 0%, rgba(251, 191, 36, 0.4) 10%, rgba(251, 191, 36, 0.4) 90%, transparent 100%)',
                    boxShadow: '0 0 15px rgba(251, 191, 36, 0.25)',
                  }}
                />
                {/* Arrow indicator on the left */}
                <div 
                  className="absolute -left-6 top-1/2 -translate-y-1/2 w-6 h-6 bg-amber-500 rounded-full flex items-center justify-center shadow-lg"
