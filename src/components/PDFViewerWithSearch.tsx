...
export function PDFViewerWithSearch({
  ...
  return (
    <div className={cn("flex flex-col h-full bg-muted/20", className)}>
      ...
      {/* PDF Container */}
      <div ref={scrollContainerRef} className="flex-1 overflow-auto flex justify-center items-start p-4 pb-4 bg-card">
        {loading && (
        ...
        >
          {documentReady && numPages > 0 && !pageError && pdfDocRef.current && (
            <div ref={pageContainerRef} className="relative inline-block mb-2">
              <Page
